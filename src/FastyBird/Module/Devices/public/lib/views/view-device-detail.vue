  ...
  <router-view v-slot="{ Component }">
    ...
    >
      <div class="fb-devices-module-view-device-detail__setting">
        ...
        <suspense>
          <component :is="Component" :connector-id="deviceData?.device.connector.id" />
...
import {
  ...
  DevicesDeviceToolbar,
  DevicesDeviceIcon,
  DeviceSettingsDeviceRemove,
} from '@/lib/components'
import { ApplicationError } from '@/lib/errors'
import {
  IChannelData,
  IDeviceData,
} from '@/types/devices-module'
...
const isPartialSettingsRoute = computed<boolean>((): boolean => {
  return route.matched.find(matched => matched.name === routeNames.deviceSettings || matched.name === routeNames.deviceSettingsAddChannel || matched.name === routeNames.deviceSettingsEditChannel) !== undefined
})
  ...
  if (validateUuid(props.id)) {
    ...
    return {
      ...
      controls: orderBy<IDeviceControl>(
        deviceControlsStore.findForDevice(device.id)
          .filter(control => !control.draft),
        [
          (v): string => v.name,
        ],
        ['asc'],
      ...
      properties: orderBy<IDeviceProperty>(
        devicePropertiesStore.findForDevice(device.id)
          .filter(control => !control.draft),
        [
      ...
      channels: orderBy<IChannelData>(
          ...
          .map((channel): IChannelData  => {
            return {
              ...
              properties: orderBy<IChannelProperty>(
                channelPropertiesStore.findForChannel(channel.id)
                  .filter(control => !control.draft),
                [
                  (v): string => v.name ?? v.identifier,
                  (v): string => v.identifier,
        ...
        [
          (v): string => v.channel.name ?? v.channel.identifier,
          (v): string => v.channel.identifier,
        ],
        ['asc'],
      ),
    }
