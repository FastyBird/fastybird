  ...
  if (validateUuid(props.id)) {
    ...
    return {
      ...
      controls: orderBy<IConnectorControl>(
        connectorControlsStore.findForConnector(connector.id)
      ...
      devices: orderBy<IDeviceData>(
        devicesStore.findForConnector(connector.id)
        ...
        [
          (v): string => v.device.name ?? v.device.identifier,
          (v): string => v.device.identifier,
        ],
        ['asc'],
      ),
    }
