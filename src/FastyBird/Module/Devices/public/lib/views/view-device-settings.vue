...
const isConnectorSettingsRoute = computed<boolean>((): boolean => {
  return route.matched.find(matched => {
    return matched.name === routeNames.connectorSettings
...
const deviceData = computed<IDeviceData>((): IDeviceData => {
  ...
  return {
    ...
    controls: orderBy<IDeviceControl>(
      deviceControlsStore.findForDevice(device.value.id)
        .filter(control => device.value?.draft ? true : !control.draft),
      [
        (v): string => v.name,
      ],
      ['asc'],
    ...
    properties: orderBy<IDeviceProperty>(
      devicePropertiesStore.findForDevice(device.value.id)
        .filter(property => device.value?.draft ? true : !property.draft),
      [
          ...
          return {
            ...
            properties: orderBy<IChannelProperty>(
              channelPropertiesStore.findForChannel(channel.id)
                .filter(property => channel.draft ? true : !property.draft),
              [
                (v): string => v.name ?? v.identifier,
                (v): string => v.identifier,
              ],
