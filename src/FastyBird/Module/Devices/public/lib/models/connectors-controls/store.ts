...
export const useConnectorControls = defineStore<string, IConnectorControlsState, IConnectorControlsGetters, IConnectorControlsActions>('devices_module_connectors_controls', {
    ...
    findByName: (state: IConnectorControlsState): ((connector: IConnector, name: string) => IConnectorControl | null) => {
      return ((connector: IConnector, name) => {
        const control = Object.values(state.data).find(control => {
          return (
            control.connector.id === connector.id
  ...
  actions: {
    ...
    async get(payload: IConnectorControlsGetActionPayload): Promise<boolean> {
      ...
      try {
        ...
        throw new ApiError(
          'devices-module.connector-controls.get.failed',
          e,
          'Fetching control failed.',
        )
      } finally {
    ...
    async fetch(payload: IConnectorControlsFetchActionPayload): Promise<boolean> {
      ...
      try {
        ...
        throw new ApiError(
          'devices-module.connector-controls.fetch.failed',
          e,
          'Fetching controls failed.',
        )
      } finally {
    ...
    async add(payload: IConnectorControlsAddActionPayload): Promise<IConnectorControl> {
      ...
      if (newControl.draft) {
        ...
        try {
          ...
          throw new ApiError(
            'devices-module.connector-controls.create.failed',
            e,
            'Create new control failed.',
          )
        } finally {
          this.semaphore.creating = this.semaphore.creating.filter(item => item !== newControl.id)
        }
    ...
    async save(payload: IConnectorControlsSaveActionPayload): Promise<IConnectorControl> {
      ...
      try {
        ...
        throw new ApiError(
          'devices-module.connector-controls.save.failed',
          e,
          'Save draft control failed.',
        )
      } finally {
    ...
    async remove(payload: IConnectorControlsRemoveActionPayload): Promise<boolean> {
      ...
      if (recordToDelete.draft) {
        ...
        try {
          await axios.delete(
            `/${ModulePrefix.MODULE_DEVICES}/v1/connectors/${recordToDelete.connector.id}/controls/${recordToDelete.id}`,
          )
        } catch (e: any) {
          const connectorsStore = useConnectors()
          ...
          throw new ApiError(
            'devices-module.connector-controls.delete.failed',
            e,
            'Delete control failed.',
          )
        } finally {
          this.semaphore.deleting = this.semaphore.deleting.filter(item => item !== payload.id)
        }
