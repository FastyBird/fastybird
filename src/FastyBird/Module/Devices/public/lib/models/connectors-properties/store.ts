...
import { useConnectors } from '@/lib/models'
...
export const useConnectorProperties = defineStore<string, IConnectorPropertiesState, IConnectorPropertiesGetters, IConnectorPropertiesActions>('devices_module_connectors_properties', {
  ...
  getters: {
    ...
    fetching: (state: IConnectorPropertiesState): ((connectorId: string | null) => boolean) => {
      return (connectorId => connectorId !== null ? state.semaphore.fetching.items.includes(connectorId) : state.semaphore.fetching.items.length > 0)
    },
    ...
    findByIdentifier: (state: IConnectorPropertiesState): ((connector: IConnector, identifier: string) => IConnectorProperty | null) => {
      return ((connector: IConnector, identifier) => {
        const property = Object.values(state.data).find(property => {
          return (
            property.connector.id === connector.id
  ...
  actions: {
    ...
    async get(payload: IConnectorPropertiesGetActionPayload): Promise<boolean> {
      ...
      try {
        ...
        throw new ApiError(
          'devices-module.connector-properties.get.failed',
          e,
    ...
    async add(payload: IConnectorPropertiesAddActionPayload): Promise<IConnectorProperty> {
      ...
      if (newProperty.draft) {
        ...
        try {
          const apiData: Partial<IConnectorProperty> = { id: newProperty.id, type: newProperty.type, identifier: newProperty.identifier, name: newProperty.name, connector: newProperty.connector, relationshipNames: ['connector'] }
    ...
    async edit(payload: IConnectorPropertiesEditActionPayload): Promise<IConnectorProperty> {
      ...
      if (updatedRecord.draft) {
        ...
        try {
          const apiData: Partial<IConnectorProperty> = { id: updatedRecord.id, type: updatedRecord.type, identifier: updatedRecord.identifier, name: updatedRecord.name, connector: updatedRecord.connector, relationshipNames: ['connector'] }
    ...
    async save(payload: IConnectorPropertiesSaveActionPayload): Promise<IConnectorProperty> {
      ...
      try {
        const apiData: Partial<IConnectorProperty> = { id: recordToSave.id, type: recordToSave.type, identifier: recordToSave.identifier, name: recordToSave.name, connector: recordToSave.connector, relationshipNames: ['connector'] }
    ...
    async remove(payload: IConnectorPropertiesRemoveActionPayload): Promise<boolean> {
      ...
      if (recordToDelete.draft) {
        ...
        try {
          await axios.delete(
            `/${ModulePrefix.MODULE_DEVICES}/v1/connectors/${recordToDelete.connector.id}/properties/${recordToDelete.id}`,
          )
        } catch (e: any) {
          const connectorsStore = useConnectors()
