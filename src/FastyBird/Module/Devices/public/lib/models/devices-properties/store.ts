...
import exchangeEntitySchema
  from '@fastybird/metadata-library/resources/schemas/modules/devices-module/entity.device.property.json'
import {
...
import { useDevices } from '@/lib/models'
import {
  IDevice,
  IPlainRelation,
...
const recordFactory = async (data: IDevicePropertyRecordFactoryPayload): Promise<IDeviceProperty> => {
  ...
  record.relationshipNames.forEach(relationName => {
    if (relationName === 'children') {
      get(data, relationName, [])
        .forEach((relation: any): void => {
...
export const useDeviceProperties = defineStore<string, IDevicePropertiesState, IDevicePropertiesGetters, IDevicePropertiesActions>('devices_module_devices_properties', {
      ...
      return ((device: IDevice, identifier) => {
        const property = Object.values(state.data).find(property => {
          return (
            ...
            && property.identifier.toLowerCase() === identifier.toLowerCase()
          )
        })
  ...
  actions: {
    ...
    async get(payload: IDevicePropertiesGetActionPayload): Promise<boolean> {
      ...
      try {
        ...
        throw new ApiError(
          'devices-module.device-properties.get.failed',
          e,
          'Fetching property failed.',
        )
      } finally {
    ...
    async add(payload: IDevicePropertiesAddActionPayload): Promise<IDeviceProperty> {
      ...
      if (newProperty.draft) {
        ...
        try {
          const apiData: Partial<IDeviceProperty> = newProperty.parent !== null ? { id: newProperty.id, type: newProperty.type, identifier: newProperty.identifier, name: newProperty.name, device: newProperty.device, parent: newProperty.parent, relationshipNames: ['device', 'parent'] } : newProperty
    ...
    async edit(payload: IDevicePropertiesEditActionPayload): Promise<IDeviceProperty> {
      ...
      if (updatedRecord.draft) {
        ...
        try {
          const apiData: Partial<IDeviceProperty> = updatedRecord.parent !== null ? { id: updatedRecord.id, type: updatedRecord.type, identifier: updatedRecord.identifier, name: updatedRecord.name, device: updatedRecord.device, parent: updatedRecord.parent, relationshipNames: ['device', 'parent'] } : updatedRecord
          ...
          const updatedProperty = await axios.patch<IDevicePropertyResponseJson>(
            ...
            jsonApiFormatter.serialize({
              stuff: updatedRecord.parent !== null ? { id: updatedRecord.id, type: updatedRecord.type, identifier: updatedRecord.identifier, name: updatedRecord.name, device: updatedRecord.device, parent: updatedRecord.parent, relationshipNames: ['device', 'parent'] } : updatedRecord,
            }),
          )
    ...
    async save(payload: IDevicePropertiesSaveActionPayload): Promise<IDeviceProperty> {
      ...
      try {
        const apiData: Partial<IDeviceProperty> = recordToSave.parent !== null ? { id: recordToSave.id, type: recordToSave.type, identifier: recordToSave.identifier, name: recordToSave.name, device: recordToSave.device, parent: recordToSave.parent, relationshipNames: ['device', 'parent'] } : recordToSave
    ...
    async remove(payload: IDevicePropertiesRemoveActionPayload): Promise<boolean> {
      ...
      if (recordToDelete.draft) {
        ...
        try {
          await axios.delete(
            `/${ModulePrefix.MODULE_DEVICES}/v1/devices/${recordToDelete.device.id}/properties/${recordToDelete.id}`,
          )
        } catch (e: any) {
          const devicesStore = useDevices()
