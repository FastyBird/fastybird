...
const recordFactory = async (data: IChannelPropertyRecordFactoryPayload): Promise<IChannelProperty> => {
  ...
  let channel = channelsStore.findById(data.channelId)
...
export const useChannelProperties = defineStore<string, IChannelPropertiesState, IChannelPropertiesGetters, IChannelPropertiesActions>('channels_module_channels_properties', {
    ...
    findByIdentifier: (state: IChannelPropertiesState): ((channel: IChannel, identifier: string) => IChannelProperty | null) => {
      return ((channel: IChannel, identifier) => {
        const property = Object.values(state.data).find(property => {
          return (
            property.channel.id === channel.id
  ...
  actions: {
    ...
    async get(payload: IChannelPropertiesGetActionPayload): Promise<boolean> {
      ...
      try {
        ...
        throw new ApiError(
          'devices-module.channel-properties.get.failed',
          e,
    ...
    async add(payload: IChannelPropertiesAddActionPayload): Promise<IChannelProperty> {
      ...
      if (newProperty.draft) {
        ...
        try {
          const apiData: Partial<IChannelProperty> = newProperty.parent !== null ? { id: newProperty.id, type: newProperty.type, identifier: newProperty.identifier, name: newProperty.name, channel: newProperty.channel, parent: newProperty.parent, relationshipNames: ['channel', 'parent'] } : newProperty
    ...
    async edit(payload: IChannelPropertiesEditActionPayload): Promise<IChannelProperty> {
      ...
      if (updatedRecord.draft) {
        ...
        try {
          const apiData: Partial<IChannelProperty> = updatedRecord.parent !== null ? { id: updatedRecord.id, type: updatedRecord.type, identifier: updatedRecord.identifier, name: updatedRecord.name, channel: updatedRecord.channel, parent: updatedRecord.parent, relationshipNames: ['channel', 'parent'] } : updatedRecord
    ...
    async save(payload: IChannelPropertiesSaveActionPayload): Promise<IChannelProperty> {
      ...
      try {
        const apiData: Partial<IChannelProperty> = recordToSave.parent !== null ? { id: recordToSave.id, type: recordToSave.type, identifier: recordToSave.identifier, name: recordToSave.name, channel: recordToSave.channel, parent: recordToSave.parent, relationshipNames: ['channel', 'parent'] } : recordToSave
