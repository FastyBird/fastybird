...
import exchangeEntitySchema
  from '@fastybird/metadata-library/resources/schemas/modules/devices-module/entity.device.attribute.json'
import {
  DeviceAttributeEntity as ExchangeEntity,
...
export const useDeviceAttributes = defineStore<string, IDeviceAttributesState, IDeviceAttributesGetters, IDeviceAttributesActions>('devices_module_devices_attributes', {
    ...
    findByIdentifier: (state: IDeviceAttributesState): ((device: IDevice, identifier: string) => IDeviceAttribute | null) => {
      return ((device: IDevice, identifier) => {
        const attribute = Object.values(state.data).find(attribute => {
          return (
            attribute.device.id === device.id
            && attribute.identifier.toLowerCase() === identifier.toLowerCase()
          )
        })
  ...
  actions: {
    ...
    async get(payload: IDeviceAttributesGetActionPayload): Promise<boolean> {
      ...
      try {
        ...
        throw new ApiError(
          'devices-module.device-attributes.get.failed',
          e,
          'Fetching attribute failed.',
        )
      } finally {
    ...
    async fetch(payload: IDeviceAttributesFetchActionPayload): Promise<boolean> {
      ...
      try {
        ...
        throw new ApiError(
          'devices-module.device-attributes.fetch.failed',
          e,
    ...
    async add(payload: IDeviceAttributesAddActionPayload): Promise<IDeviceAttribute> {
      ...
      if (newAttribute.draft) {
        ...
        try {
          ...
          throw new ApiError(
            'devices-module.device-attributes.create.failed',
            e,
            'Create new attribute failed.',
          )
        } finally {
          this.semaphore.creating = this.semaphore.creating.filter(item => item !== newAttribute.id)
        }
    ...
    async save(payload: IDeviceAttributesSaveActionPayload): Promise<IDeviceAttribute> {
      ...
      try {
        ...
        throw new ApiError(
          'devices-module.device-attributes.save.failed',
          e,
          'Save draft attribute failed.',
        )
      } finally {
    ...
    async remove(payload: IDeviceAttributesRemoveActionPayload): Promise<boolean> {
      ...
      if (recordToDelete.draft) {
        ...
        try {
          await axios.delete(
            `/${ModulePrefix.MODULE_DEVICES}/v1/devices/${recordToDelete.device.id}/attributes/${recordToDelete.id}`,
          )
        } catch (e: any) {
          const devicesStore = useDevices()
          ...
          throw new ApiError(
            'devices-module.device-attributes.delete.failed',
            e,
