...
import exchangeEntitySchema
  from '@fastybird/metadata-library/resources/schemas/modules/devices-module/entity.device.control.json'
import {
...
export const useDeviceControls = defineStore<string, IDeviceControlsState, IDeviceControlsGetters, IDeviceControlsActions>('devices_module_devices_controls', {
  ...
  actions: {
    ...
    async get(payload: IDeviceControlsGetActionPayload): Promise<boolean> {
      ...
      try {
        ...
        throw new ApiError(
          'devices-module.device-controls.get.failed',
          e,
    ...
    async add(payload: IDeviceControlsAddActionPayload): Promise<IDeviceControl> {
      ...
      if (newControl.draft) {
        ...
        try {
          ...
          throw new ApiError(
            'devices-module.device-controls.create.failed',
            e,
    ...
    async save(payload: IDeviceControlsSaveActionPayload): Promise<IDeviceControl> {
      ...
      try {
        ...
        throw new ApiError(
          'devices-module.device-controls.save.failed',
          e,
    ...
    async remove(payload: IDeviceControlsRemoveActionPayload): Promise<boolean> {
      ...
      if (recordToDelete.draft) {
        ...
        try {
          await axios.delete(
            `/${ModulePrefix.MODULE_DEVICES}/v1/devices/${recordToDelete.device.id}/controls/${recordToDelete.id}`,
          )
        } catch (e: any) {
          const devicesStore = useDevices()
