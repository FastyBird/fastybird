...
import { isString } from '@fastybird/web-ui-utils';
...
export default defineComponent({
	...
	setup(props, { emit }) {
		...
		watch(
			(): HTMLElement | undefined => props.focusTrapEl,
		...
		const releaseOnFocus = (e: Event): void => {
			emit(ON_RELEASE_FOCUS_EVT, e);
		};
		...
		const startTrap = async (): Promise<void> => {
		...
		const stopTrap = (): void => {
			...
			if (trapContainer) {
				...
				if (
					...
					(focusReason.value == 'keyboard' || !isFocusCausedByUserEvent() || trapContainer.contains(document.activeElement))
				) {
