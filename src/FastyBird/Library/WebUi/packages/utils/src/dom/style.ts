...
export const classNameToArray = (cls = ''): string[] => cls.split(' ').filter((item) => !!item.trim());
...
export const setStyle = (element: HTMLElement, styleName: CSSProperties | keyof CSSProperties, value?: string | number): void => {
...
export const removeStyle = (element: HTMLElement, style: CSSProperties | keyof CSSProperties): void => {
	if (!element || !style) {
		return;
	}
