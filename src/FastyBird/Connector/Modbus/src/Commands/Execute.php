...
class Execute extends Console\Command\Command
{
	...
	protected function execute(Input\InputInterface $input, Output\OutputInterface $output): int
	{
		...
		if ($input->getOption('quiet') === false) {
			$io->title((string)$this->translator->translate('//modbus-connector.cmd.execute.title'));

			$io->note((string)$this->translator->translate('//modbus-connector.cmd.execute.subtitle'));
		}
		...
		if (
			...
			if ($connector === null) {
				$io->warning(
					(string) $this->translator->translate('//modbus-connector.cmd.execute.messages.connector.notFound'),
				);
			...
			if (count($connectors) === 0) {
				$io->warning(
					(string) $this->translator->translate('//modbus-connector.cmd.base.messages.noConnectors'),
				);
			...
			if (count($connectors) === 1) {
				...
				if ($connector === null) {
					$io->warning(
						(string) $this->translator->translate(
							'//modbus-connector.cmd.execute.messages.connector.notFound',
						),
					);
				...
				if ($input->getOption('no-interaction') === false) {
					$question = new Console\Question\ConfirmationQuestion(
						(string) $this->translator->translate(
							'//modbus-connector.cmd.execute.questions.execute',
				...
				$question = new Console\Question\ChoiceQuestion(
					(string) $this->translator->translate('//modbus-connector.cmd.execute.questions.select.connector'),
		...
		if (!$connector->isEnabled()) {
			$io->warning(
				(string) $this->translator->translate('//modbus-connector.cmd.execute.messages.connector.disabled'),
			);
		...
		if ($result !== Console\Command\Command::SUCCESS) {
			$io->error((string) $this->translator->translate('//modbus-connector.cmd.execute.messages.error'));
